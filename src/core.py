...
class Config:
    ...
    def generate_positions_bs_dict(self):
        ...
        for bs_id in ["21", "22", "23", "24"]:
            if bs_id == "21":
                bs_gps = [0.0, 0.0, 10.0]
                bs_pose = [45.0, 0.0, 0.0]
            elif bs_id == "22":
                bs_gps = [0.0, 100.0, 10.0]
                bs_pose = [45.0, 0.0, 0.0]
            elif bs_id == "23":
                bs_gps = [100.0, 100.0, 10.0]
                bs_pose = [45.0, 0.0, 0.0]
            elif bs_id == "24":
                bs_gps = [100.0, 0.0, 10.0]
                bs_pose = [45.0, 0.0, 0.0]
            
            # Position relative to Testbed reference
            enu = np.array(bs_gps)
            positions_bs_dict[bs_id] = {
                "enu": enu,
                "xyz": np.array([enu[0], enu[1], enu[2]]),
                "pose": bs_pose
            }
        return positions_bs_dict
        # Logic to generate position dict according to bs_id
cfg = Config()
...
class PointTarget:
    ...
    Attributes:
        ...
        avg_rcs (float): target average RCS (m^2)
        swerling_model (str): Swerling model type ('Swerling0', 'Swerling1', 'Swerling2', 'Swerling3', 'Swerling4')
    Methods:
        get_rcs(): Get current RCS value
        refresh_rcs(): Refresh RCS value (for Swerling 1 and 3 models)
        update(time_elapsed): Update target position and RCS value
    """
    def __init__(self, trajectory, avg_rcs, swerling_model='Swerling0'):
        self.trajectory = trajectory  # Pass the above trajectory instance
...
class OscillatingTrajectory(Trajectory):
    """Simple harmonic motion/Sine wave (Simulates UAV hover jitter or patrol)"""
...
class DistributedTarget(PointTarget):
    """
    Simulates distributed targets with physical dimensions.
     [Translated] （Sub-scatterers） [Translated] 。
    """
...
def synthesize_with_calibration(cfg, targets, bg_iq, ref_peak_power, target_snr_db=15.0):
    ...
    """
    # 1.  RV 
    # ：calibrate_rv_matrix_adaptive  RV 
    ...
    # ：exp(j * 2 * pi * k * offset / N)
    k = np.arange(cfg.number_subcarriers)[:, None] #  (1024, 1)
    # ：k_offset  IFFT ，
...
def synthesize_simtarget_plus_gaussian_noise(cfg, targets, target_snr_db=15.0, noise_variance=1.0):
    ...
    Args:
        cfg:  [Translated] 
        ref_peak_power:  [Translated] (RCS=1) [Translated] ， [Translated] 
        target_snr_db:  [Translated] RD [Translated]  (dB)
        noise_variance:  [Translated] 
        
    Returns:
        sim_target_plus_noise_iq_data:  [Translated] IQ [Translated] 
    """
    # 1.  ()
    #  P_n = E[|n|^2] = 2 * sigma^2 ( sigma^2)
    # 
...
def ca_cfar_2d(rv_map, guard_cells=(2, 2), training_cells=(5, 5), pfa=1e-6):
    """
     [Translated] 2D RV [Translated] CA-CFAR [Translated] 。

    Args:
        rv_map: 2D numpy array,  [Translated]  ( [Translated] dB)
        guard_cells: (g_r, g_d)  [Translated] 
        training_cells: (t_r, t_d)  [Translated] 
        pfa:  [Translated]  (Probability of False Alarm)
        
    Returns:
        detections: 2D boolean array, True [Translated] 
        threshold_map: 2D array,  [Translated] 
    """
...
def add_window_to_channel_matrix(radar_frame, window_type='hamming'):
    ...
    if window_type == 'hamming':
        w_range = np.hamming(n_sc)[:, None] # 
        w_doppler = np.hamming(n_sym)[None, :] # 
    else:
...
def channel_to_rv_map_windowed(radar_frame, window_type='hamming'):
    """
     [Translated]  RV Map  [Translated] 。
    Args:
        radar_frame:  [Translated] 
        window_type: 'hamming', 'hann', 'blackman'  [Translated] 
    """
...
def calibrate_rv_matrix_adaptive(rv_matrix):
    """
     [Translated]  RV  [Translated] ， [Translated] 。
    
     [Translated] ：
    1.  [Translated] ： [Translated]  RV  [Translated] （Doppler） [Translated] ， [Translated] “ [Translated] ”。
    2.  [Translated] ： [Translated] （ [Translated] ）。
    3.  [Translated] ： [Translated]  k_offset。
    4.  [Translated] ： [Translated]  numpy.roll  [Translated] ， [Translated]  k_offset  [Translated]  0  [Translated] 。
    """
    # 1.  (Projection)
    range_profile = np.mean(rv_matrix, axis=1)
    
    # 2.  & 3.  (Feature Extraction & Calculate Offset)
    # “0”
    k_offset = np.argmax(range_profile)
...
def plot_2d_range_doppler(cfg, rd_map: np.ndarray, plot_dir: Path | str,  filename: str, cbar_label='Normalized Power (dB)', title='2D Range-Doppler Map'):
    """ [Translated] 2D [Translated] - [Translated] 。"""
...
def plot_cn_2d_range_doppler(cfg, rd_map: np.ndarray, plot_dir: Path | str, title : str, filename: str, cbar_label=' [Translated]  (dB)'):
    """ [Translated] 2D [Translated] - [Translated] 。"""
    plot_dir = Path(plot_dir)
    plot_dir.mkdir(parents=True, exist_ok=True)
    fig, ax = plt.subplots(figsize=(10, 6))
    ...
    fig.colorbar(im, ax=ax, label=cbar_label)
    ax.set_title(title)
    ax.set_xlabel(' [Translated]  (m/s)')
    ax.set_ylabel(' [Translated]  (m)')
...
def plot_cfar_results(cfg, rv_map: np.ndarray, detections: np.ndarray, plot_dir: Path | str, title: str, filename: str, targets=None):
    """
     [Translated] CFAR [Translated] ， [Translated] Ground Truth。

    Args:
        cfg:  [Translated] 
        rv_map: 2D RD [Translated] 
        detections:  [Translated] 
        title:  [Translated] 
        filename:  [Translated] 
        targets: (Optional)  [Translated] ， [Translated] Ground Truth
    """
    fig, ax = plt.subplots(figsize=(10, 6))
    plot_dir = Path(plot_dir)
    # 1. RD
...
def plot_cn_cfar_results(cfg, rv_map, detections, title, filename, targets=None):
    ...
    im = ax.imshow(log_rv, aspect='auto', cmap='plasma', origin='lower',
                   extent=[cfg.velocity_seq[0], cfg.velocity_seq[-1], cfg.ranges_seq[0], cfg.ranges_seq[-1]],vmax=0,vmin=-140)
    fig.colorbar(im, ax=ax, label=' [Translated]  (dB)')
    ...
    if len(det_indices) > 0:
        ...
        #  'x' ， size 
        ax.scatter(det_velocities, det_ranges, c='black', marker='x', s=40, label='CFAR  [Translated] ', alpha=0.9)
    ...
    if targets is not None:
        ...
        #  GT， (facecolors='none')
        ax.scatter(gt_velocities, gt_ranges, edgecolors='red', facecolors='none', 
                   marker='o', s=150, linewidths=2, label=' [Translated] ')
